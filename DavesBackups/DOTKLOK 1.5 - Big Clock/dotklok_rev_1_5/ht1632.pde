...
// scroll_text(char *string)
// scroll a string of text across the screen
// at a random speed and in a random row
// uses myfont in font.h from original ht1632 library by WestfW
//   (slightly modified by Andrew O'Malley)
void scroll_text(char *string)
{
  //char *string="2011 Ingenious Award";

  unsigned long time;

  int del=random(20,90); // scroll delay

  byte y=random(0,Y_MAX+1-7); // what row to display the text in
  byte x=X_MAX;               // the starting column for displaying a new letter

  int pos=0; // current position in the string
  char c; // the actual character (at the current postion of string)
  char col=0; // current column in the current letter/position
  byte dots; // the dots that make up one column in a letter
  ...
  while(string[pos]!='\0') {
    ...
    // constrain the character to the font
    if (c >= 'A' && c <= 'Z' || (c >= 'a' && c <= 'z') ) {
      c &= 0x1F;   // A-Z maps to 1-26
    } 
    else if (c >= '0' && c <= '9') {
      ...
      c = 0; // space for every other character
    }

    if(col<5) { // shifting in a letter  
    ...
    else { // get the next letter
      pos++;
      col=0;
      x=X_MAX; // start it at the right edge of the screen
    }
  ...
  } // end while(pos < length of string

  // after reading string, scroll to clear
