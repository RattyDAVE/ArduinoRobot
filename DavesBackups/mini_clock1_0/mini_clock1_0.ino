...
#include <Dave_Button.h>                      // Button library by Alexander Brevig


// Setup LED Matrix

// pin D7 is connected to the DataIn on the display
// pin D4 is connected to the CLK on the display
// pin D5 is connected to LOAD on the display
LedControl lc = LedControl(D7, D4, D5, 4); //sets the 3 pins as D7, D4 & D5 and then sets 4 displays (max is 8 displays)
...
Button buttonA = Button(D3, PULLUP);      // Setup button A (using button library)
Button buttonB = Button(D9, PULLUP);      // Setup button B (using button library)

void setup() {

  //digitalWrite(D3, HIGH);                 // turn on pullup resistor for button on pin 2
  //digitalWrite(D9, HIGH);                 // turn on pullup resistor for button on pin 3
// DAVE ??? WHATS THIS FOR?  digitalWrite(4, HIGH);                 // turn on pullup resistor for button on pin 4
